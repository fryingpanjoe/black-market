...
class FlowControl(object):
    MIN_RATE = 10.0
    MAX_RATE = 30.0
    MIN_PENALTY_TIME = 1.0
    MAX_PENALTY_TIME = 60.0
    MIN_GOOD_TIME = 10.0
    PENALTY_THRESHOLD = 10.0
    RTT_THRESHOLD = 250.0

    def __init__(self):
        # flag for good or bad mode
        self.is_good = True

        # good rtt timer
        self.good_time = 0.0

        # time spent in good mode before we reduce the penalty time
        self.deduct_time = 0.0

        # time to spend in bad mode if we go above the rtt theshold
        self.penalty_time = 4.0

    def update_rtt(self, time_delta, rtt):
        if self.is_good:
            # good mode
            if rtt < self.RTT_THRESHOLD:
                # good rtt
                self.good_time += time_delta
                self.deduct_time += time_delta
                if self.deduct_time > self.PENALTY_THRESHOLD:
                    # reduce penalty since we have been good
                    self.deduct_time = 0.0
                    self.penalty_time = max(
                        self.penalty_time / 2.0, self.MIN_PENALTY_TIME)
            else:
                # bad rtt
                self.is_good = False
                if self.good_time < self.MIN_GOOD_TIME:
                    # increase penalty to smooth out jumps
                    self.penalty_time = min(
                        self.penalty_time * 2.0, self.MAX_PENALTY_TIME)
                self.good_time = 0.0
                self.deduct_time = 0.0
        else:
            # bad mode
            if rtt < self.RTT_THRESHOLD:
                ...
                # bad rtt
                self.good_time = 0.0
            if self.good_time > self.penalty_time:
                # go back to good mode since we have been good long enough
                self.is_good = True
                self.good_time = 0.0
                self.deduct_time = 0.0

    def get_rate(self):
        return self.MAX_RATE if self.is_good else self.MIN_RATE
...
class Channel(object):
    ...
    def __init__(self, sock):
        ...
        # true if we're waiting for the next message
        self.waiting_for_next_message = True
    ...
    def send_all_events(self):
        ...
        for event in self.out_events:
            ...
            if not message_writer.write_string(serialized_event):
                ...
                message_writer = WriteBuffer(self.MAX_MESSAGE_SIZE)
                if not message_writer.write_string(serialized_event):
                    raise NetworkError('Failed to write event')
    ...
    def send_data(self):
        try:
            ...
            if not self.write_buffer.is_empty():
                ...
                _, writable, _ = select([], [self.sock], [], 0)
                if writable:
                    # send data!
    ...
    def on_data_received(self):
        # read message id
        #if self.waiting_for_next_message:
        #    message_id = self.read_buffer.read_int32()
        #    if message_id:
        #        if self.recv_message_id is not None:
        #            if self.recv_message_id != (message_id - 1):
        #                raise NetworkError(
        #                    'Out of sync %d + 1 != %d' %
        #                    (self.recv_message_id, message_id))
        #        # we have a new message
        #        self.recv_message_id = message_id
        #        self.waiting_for_next_message = False

        # read message data
        #if not self.waiting_for_next_message:
...
class Server(object):
    ...
    def accept_pending_clients(self):
        readable, _, _ = select([self.server_socket], [], [], 0)
        if readable:
    ...
    def read_from_clients(self):
        readable, _, _ = select(self.client_sockets, [], [], 0)
    ...
    def write_to_clients(self):
        _, writable, _ = select([], self.client_sockets, [], 0)
